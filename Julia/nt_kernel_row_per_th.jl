...
function countTri_row_per_th_nt(csr_rows, col_indx, out_sum)
    ...
    if tid <= len
        a  = 1
        b  = 1
        @inbounds blen = sh_len[tid]
        while true
            if a > len || b > blen
                break;
            end

            @inbounds b1 = sh_row[a] == sh_cols[tid,b];
            @inbounds b2 = sh_row[a] > sh_cols[tid,b];
            @inbounds b3 = sh_row[a] < sh_cols[tid,b];
...
function CountTriangles_RowPerTh_nt(csr_rows, col_indx, max_per_row)
    ...
    @cuda(blocks=(512,512,blocks), threads=(32, th_groups,1),
        ...
        countTri_row_per_th_nt(dev_csr_rows, dev_col_indx, dev_out_sum))
