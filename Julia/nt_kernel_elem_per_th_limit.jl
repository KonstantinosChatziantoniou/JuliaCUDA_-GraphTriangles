...
#function countTri(out_sum)
    ...
    sh_cols = @cuDynamicSharedMem(Int32, max_nnz + CUDAnative.max(Int32(stride), Int32(max_nnz)),
                                sizeof(Int32)*max_mem_nnz)
    sh_sum = @cuDynamicSharedMem(Int32, blockDim().y,
            (max_nnz + CUDAnative.max(Int32(stride), Int32(max_nnz)) + max_mem_nnz)*sizeof(Int32))
